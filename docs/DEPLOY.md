...
| OpenAI | Point `LLM_BASE_URL` / `LLM_MODEL` at a current OpenAI text model with a real `LLM_API_KEY`. Note: OCR via OpenAI Vision is not pin-compatible with the `MinerU2.5-Pro` output format that the parser expects — verify on a sample PDF before swapping. |
